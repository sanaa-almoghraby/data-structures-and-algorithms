...
const toLastNames = people => {
  // Solution code here...
  let newArr = people.map(el => `${el.firstName} ${el.lastName}`);
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let newArr= arr.reduce((acc,val,index)=>index+1 ,0)
  return newArr;
};
...
const returnNames = (arr) => {
  // Solution code here...
  let newArr=arr.reduce((acc,val,indx )=>{
    acc[indx]=val.name
    return acc;
  },[])
return newArr;
...
const reversedString = (str) => {
  ...
  let reverse = newStr.reduce((acc, val) => val+ acc, '');
  return reverse;
  // Solution code here...
};
