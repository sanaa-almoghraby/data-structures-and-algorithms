...
const checkValues = (obj, value) => {
  ...
  let val2=val.includes(value)?true:false;
  return val2;
};
...
const updateNumbers = (obj) => {
  ...
  let newArr= name.map((name,i) => `${name}: ${phone[i]}`);
  return newArr;
...
const getHouses = (arr) => {
  ...
  houses = arr.map(object => object.house)
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.map(el => {
    if (el.name === character) {
      if (Object.values(el).includes(el.children)) {
        value = true;
      } else {
        value = false;
      }
