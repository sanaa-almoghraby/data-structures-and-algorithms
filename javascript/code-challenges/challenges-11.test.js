...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArr=input.map(ele=>ele.filter(ele=>!(ele%5)));
  let notStar=newArr.map(ele=>ele.filter(ele=>typeof(ele) !=='string'));
  let result=notStar.map(ele=>ele.map(ele=>Math.pow(2,ele)));
  return result;
};
...
let findMaleAndFemale = (data) => {
  ...
  let newArr=data.filter(el=>el.gender==='female' || el.gender==='male').map(el=>el.name).join(' and ');
  return newArr;
};
